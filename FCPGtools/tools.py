...
def tauDrainDir(inRast: str, outRast: str, band: int = 1, updateDict: dict = None, verbose: bool = False) -> None:
    ...
    updateDict : dict (optional)
        Dictionary of Rasterio raster options used to create outRast.
        Defaults have been supplied, but may not work in all situations and input file formats.
...
def ESRIDrainDir(inRast: str, outRast: str, band: int = 1, updateDict: dict = None, verbose: bool = False) -> None:
...
def accumulateParam(paramRast: str, fdr: str, accumRast: str,
                    outNoDataRast: str = None, outNoDataAccum: str = None, zeroNoDataRast: str = None, cores: int = 1,
                    mpiCall: str = 'mpiexec', mpiArg: str = '-n', verbose: bool = False) -> None:
    ...
    This function will then save a raster with all no data values set to one and other values set to zero (outNoDataRast)
    and use tauDEM to accumulate it (outNoDataAccum). It will then accumulate the parameter from the zeroNoDataRast,
    and a subsequent correction will be needed in the make_fcpg() function based on the values in the outNoDataAccum raster.
...
def make_fcpg(accumParam: str, fac: str, outRast: str,
              noDataRast: str = None, minAccum:float = None, ESRIFAC: bool = False, verbose: bool = False) -> None:
    """
    Create a flow-conditioned parameter grid using accumulated parameter and area rasters.
    See also :py:func:`make_fcpg_batch`.
    ...
    ESRIFAC : bool (optional)
        Use an ESRI FAC grid, defaults to False.
        ESRI-derived FAC grids have zeros for the first cell in a flowpath, as such, a 1 is added to the grid.
    verbose : bool (optional)
        Print output, defaults to False.
...
def resampleParam(inParam: str, fdr: str, outParam: str,
                  resampleMethod: str = "bilinear", cores: int = 1, forceProj: bool = False,
                  forceProj4: str = "\"+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs\"",
                  verbose: bool = False) -> None:
    """
    Resample, re-project, and clip the parameter raster based on the resolution, projection, and extent of the flow
    direction raster supplied. See also :py:func:`resampleParams`.
    ...
    if str(paramcrs) == str(
        ...
        try:
            ...
            cmd = 'gdalwarp -overwrite -tr {xsize} {ysize} -t_srs {fdrcrs} -te {fdrXmin} {fdrYmin} {fdrXmax}' \
                  ' {fdrYmax} -co "PROFILE=GeoTIFF" -co "TILED=YES" -co "SPARSE_OK=TRUE" -co "COMPRESS=LZW"' \
                  ' -co "ZLEVEL=9" -co "NUM_THREADS={cores}" -co "BIGTIFF=IF_SAFER" -r {resampleMethod}' \
                  ' -dstnodata {nodata} -ot {datatype} {inParam} {outParam}'.format(**warpParams)
            if verbose: print(cmd)
...
def makeDecayGrid(d2strm: str, k: float, outRast: str, verbose: bool = False) -> None:
    """
    Create a decay raster where grid cell values are computed as the inverse number of grid cells,
    :code:`np.exp((-1 * decayGrid * xsize) / (xsize ** k))`, where decayGrid is the distance from
    the d2strm raster from each grid cell to the nearest stream, k is a constant applied to the
    cell size values, and dx is the cell size of the raster, pulled from the d2strm raster directly.
...
def applyMult(inRast: str, mult: str, outRast: str, verbose: bool = False) -> None:
    """
...
def decayAccum(ang: str, mult: str, outRast: str,
               paramRast: str = None, cores: int = 1,
               mpiCall: str = 'mpiexec', mpiArg: str = '-n', verbose: bool = False) -> None:
    """
...
def dist2stream(fdr: str, fac: str, thresh: int, outRast: str,
                cores: int = 1, mpiCall: str = 'mpiexec', mpiArg: str = '-n', verbose: bool = False) -> None:
    """
...
def maskStreams(inRast: str, streamRast: str, outRast: str, verbose: bool = False) -> None:
    """
...
def resampleParam_batch(inParams: str, fdr: str, outWorkspace: str,
                        resampleMethod: str = "bilinear", cores: int = 1, appStr: str = "rprj", forceProj: bool = False,
                        forceProj4: str = "\"+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs\"",
...
def make_fcpg_batch(accumParams: list, fac: str, outWorkspace: str,
                    minAccum: int = None, appStr: str = "FCPG", ESRIFAC: bool = False, verbose: bool = False) -> list:
    """
...
def cat2bin(inCat: str, outWorkspace: str, par: bool = True, verbose: bool = False) -> list:
    """
...
def binarizeCat(val: int, data: np.ndarray, nodata: Union[int, float], outWorkspace: str, baseName: str, ext: str,
                profile: dict, verbose: bool = False) -> str:
    """
    Turn a categorical raster (e.g. land cover type) into a set of binary rasters, one for each category in the
    supplied raster, zero for areas where that class is not present, 1 for areas where that class is present,
    and -1 for regions of no data in the supplied raster. See also :py:func:`cat2bin`.
...
def tauFlowAccum(fdr: str, accumRast: str,
                 cores: int = 1, mpiCall: str = 'mpiexec', mpiArg: str = '-n', verbose: bool = False) -> None:
...
def ExtremeUpslopeValue(fdr: str, param: str, output: str,
                        accum_type: str = "MAX", cores: int = 1, fac: str = None, thresh: int = None,
                        mpiCall: str = 'mpiexec', mpiArg: str = '-n', verbose: bool = False) -> None:
    """
...
def getFeatures(gdf: gpd.GeoDataFrame) -> list:
    """
...
def makePourBasins(wbd: gpd.GeoDataFrame, fromHUC4: str, toHUC4: str,
                   HUC12Key: str = 'HUC12', ToHUCKey: str = 'ToHUC') -> gpd.GeoDataFrame:
    """
...
def findPourPoints(pourBasins: gpd.GeoDataFrame, upfacfl: str, upfdrfl: str, plotBasins: bool = False) -> list:
    """
...
def loadRaster(fl: str, returnMeta: bool = False, band: int = 1) -> np.ndarray:
    """
...
def findLastFACFD(facfl: str, fl: str = None) -> list[float, float, float, float]:
    """
...
def queryPoint(x: Union[float, int], y: Union[float, int], grd: str) -> Union[float, int]:
    """
...
def FindDownstreamCellTauDir(d: int, x: float, y: float, w: float) -> tuple:
...
def saveJSON(dictionary: dict, outfl: str) -> None:
...
def createUpdateDict(x: list, y: list, upstreamFACmax: list, fromHUC: str, outfl: str,
                     replaceDict: bool = True, verbose: bool = False, outletX: list = None, outletY: list = None) -> dict:
    """
...
def updateRaster(x: Union[list, float], y: Union[list, float], val: Union[int, float],
                 grd: str, outgrd: str, scaleFactor: int = None) -> None:
...
def makeFACweight(ingrd: str, outWeight: str) -> None:
    """
...
def adjustFAC(facWeighttemplate: str, downstreamFACweightFl: str, updateDictFl: str, downstreamFDRFl: str, adjFACFl: str,
              cores: int = 1, verbose: bool = False, scaleFactor: int = None, moveDownstream: bool = False) -> None:
    """
    ...
    verbose : bool (optional)
        Print output, defaults to False.
    scaleFactor : int (optional)
        Value to divide weighting grid by for working with very large flow accumulation values and associated
        parameter value, defaults to None.
    moveDownstream : bool (optional)
        Move the pour point downstream to account for pour points that do not overlap downstream geospatial
...
def updateDict(ud: str, upHUC: str, varName: str, val: Union[list, int, float]) -> None:
    """
...
def adjustParam(updatedParam: str, downstreamParamFL: str, updateDictFl: str, adjParamFl: str,
                verbose: bool = False, scaleFactor: int = None, moveDownstream: bool = False) -> None:
    """
...
def d8todinfinity(inRast: str, outRast: str, updateDict: dict = None, verbose: bool = False) -> None:
    """
...
def changeNoData(inRast: str, newNoData: str, updateDict: dict = None, verbose: bool = False) -> None:
    """
...
def makeStreams(fac: str, strPath: str, thresh: int = 900, updateDict: dict = None, verbose: bool = False) -> None:
    """
    ...
    strPath : str
        Path to output the stream grid where stream cells will be 1 and other cells will be the no-data value
        from the source fac grid.
    thresh : int (optional)
